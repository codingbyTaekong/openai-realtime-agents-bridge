...
class AudioProcessor {
    constructor() {
        this.DEFAULT_SAMPLE_RATE = 24000;
        this.DEFAULT_CHANNELS = 1;
        this.MAX_AUDIO_SIZE = parseInt(process.env.MAX_AUDIO_SIZE || '10485760'); // 10MB 기본값
    }
    /**
     * 입력 오디오 데이터 처리
     */
    async processAudioInput(audioData, format) {
        try {
            let buffer;
            // 데이터 타입 변환
            if (typeof audioData === 'string') {
                // Base64 디코딩
                buffer = buffer_1.Buffer.from(audioData, 'base64');
            }
            else {
                buffer = audioData;
            }
     ...
     * 오디오 메타데이터 추출
     */
    getAudioMetadata(buffer, format) {
        const metadata = {
            size: buffer.length,
            format: format
        };
