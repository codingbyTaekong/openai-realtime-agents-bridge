...
class OpenAIService {
     ...
     * 실시간 세션 생성 및 설정
     */
    async createRealtimeSession(sessionId, config) {
        try {
            const defaultConfig = {
                model: "gpt-4o-realtime-preview-2025-06-03",
                voice: "sage",
     ...
     * 텍스트 메시지를 OpenAI에 전송
     */
    async sendTextMessage(sessionId, message) {
        try {
     ...
     * 오디오 메시지를 OpenAI에 전송
     */
    async sendAudioMessage(sessionId, audioData) {
        try {
            const session = this.sessions.get(sessionId);
            if (!session) {
                throw new Error('세션을 찾을 수 없습니다.');
     ...
     * 함수 호출 처리
     */
    async executeFunctionCall(functionName, arguments_) {
        try {
     ...
     * 사용자 계정 정보 조회 (샘플 데이터)
     */
    getUserAccountInfo(phoneNumber) {
    ...
    lookupPolicyDocument(topic) {
        ...
        return sampleDocs.filter(doc => doc.topic.toLowerCase().includes(topic.toLowerCase()) ||
            doc.content.toLowerCase().includes(topic.toLowerCase()));
    }
    /**
     * 가장 가까운 매장 찾기 (샘플 데이터)
     */
    findNearestStore(zipCode) {
