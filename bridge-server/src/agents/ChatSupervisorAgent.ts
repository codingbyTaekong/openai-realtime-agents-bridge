...
export class ChatSupervisorAgent {
  private openaiService: OpenAIService;
  private whisperService: WhisperService;
  private readonly companyName = 'NewTelco';
  ...
  async processTextMessage(
    ...
    try {
      ...
      const supervisorResponse = await this.getNextResponseFromSupervisor(
        userMessage,
        conversationHistory
      );
  ...
  async processAudioMessage(
    ...
    try {
      ...
      // 오디오 품질 검사
      const audioFormat = (audioMessage as any).format || 'wav';
   ...
   * 직접 처리 가능한 메시지인지 확인
   */
  private shouldHandleDirectly(message: string): boolean {
   ...
   * 직접 응답 처리
   */
  private handleDirectResponse(message: string): string {
    const lowerMessage = message.toLowerCase().trim();
   ...
   */
  private async getNextResponseFromSupervisor(
...
${relevantContext}`
   ...
   * 도구 호출 처리
   */
  private async handleToolCalls(body: any, response: any): Promise<string | { error: string }> {
   ...
   * 도구 응답 가져오기
   */
  private getToolResponse(functionName: string, args: any): any {
    switch (functionName) {
      case "getUserAccountInfo":
        return this.sampleAccountInfo;
      
      case "lookupPolicyDocument":
        return this.samplePolicyDocs.filter(doc => 
          doc.topic.toLowerCase().includes(args.topic?.toLowerCase() || '') ||
          doc.content.toLowerCase().includes(args.topic?.toLowerCase() || '')
        );
      
      case "findNearestStore":
        return this.sampleStoreLocations.filter(store =>
          store.zip_code === args.zip_code
        );
   ...
   * 필러 문구 랜덤 선택
   */
  private getRandomFillerPhrase(): string {
    const fillerPhrases = [
      "Just a second.",
      "Let me check.",
      "One moment.",
   ...
   */
  private getSupervisorInstructions(): string {
...
- Only provide information about this company, its policies, its products, or the customer's account, and only if it is based on information provided in context. Do not answer questions outside this scope.`;
   ...
   */
  private getSupervisorTools(): any[] {
    return [
      {
        type: "function",
